...
export class CodexThreadStore {
  ...
  upsert(metadata: CodexThreadMetadata): void {
    this.database.prepare(`
      INSERT INTO codex_threads(thread_id, project_path, title, status, last_event_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    ...
    `).run(metadata.threadId, metadata.projectPath, metadata.title, metadata.status, metadata.lastEventAt, metadata.createdAt);
  }
  ...
  private fromRow(row: ThreadRow): CodexThreadMetadata {
    return { threadId: row.thread_id, projectPath: row.project_path, title: row.title, status: row.status, lastEventAt: row.last_event_at, createdAt: row.created_at };
  }
}
