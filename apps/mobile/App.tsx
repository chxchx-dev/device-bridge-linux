...
import { BridgeClient, type Action, type CodexApprovalMetadata, type CodexGatewayStatus, type CodexProject, type CodexTaskEvent, type CodexThreadMetadata, type DeviceStatus, type IntegrationStatus, type Mode, type ModeStatus } from './src/api/bridge-client';
...
export default function App() {
  ...
  const forget = async () => { await clearSession(); setSession(undefined); setStatus(undefined); setActions([]); setModeStatus(undefined); setCodexStatus(undefined); setCodexThreads([]); setCodexProjects([]); setCodexApprovals([]); setCodexEvents([]); setCodexResponse(''); setSelectedThread(undefined); setMessage('Session removed from secure storage.'); };
