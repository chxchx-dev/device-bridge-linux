...
import { BridgeClient, type Action, type CodexApprovalMetadata, type CodexGatewayStatus, type CodexProject, type CodexTaskEvent, type CodexThreadMetadata, type DeviceStatus, type IntegrationStatus, type Mode, type ModePlan, type ModeStatus, type RegisteredServiceStatus } from './src/api/bridge-client';
...
function renderCodexMarkdown(markdown: string) {
  ...
  return lines.map((line, index) => {
    ...
    if (inCode) return <Text key={`code-${index}`} style={styles.responseCode}>{line || ' '}</Text>;
    const heading = /^(#{1,3})\s+(.+)$/.exec(line);
    if (heading) return <Text key={`heading-${index}`} style={heading[1].length === 1 ? styles.responseHeading : styles.responseSubheading}>{heading[2]}</Text>;
    const bullet = /^\s*[-*]\s+(.+)$/.exec(line);
    if (bullet) return <Text key={`bullet-${index}`} style={styles.responseList}>• {bullet[1]}</Text>;
    const numbered = /^\s*\d+[.)]\s+(.+)$/.exec(line);
    if (numbered) return <Text key={`number-${index}`} style={styles.responseList}>{line.trim()}</Text>;
    if (!line.trim()) return <View key={`space-${index}`} style={styles.responseSpacer} />;
    return <Text key={`paragraph-${index}`} style={styles.responseText}>{line}</Text>;
  });
}
...
export default function App() {
  ...
  const [modePreview, setModePreview] = useState<ModePlan>();
  const [serviceStatus, setServiceStatus] = useState<RegisteredServiceStatus[]>([]);
  ...
  const refresh = useCallback(async (activeSession: StoredSession) => {
    ...
    if (catalog.actions.some((action) => action.id === 'mode.preview')) {
      try { setModePreview((await client.getModePreview(activeSession, modeStatus?.mode === 'game' ? 'dev' : 'game')).result); } catch { setModePreview(undefined); }
    } else setModePreview(undefined);
    if (catalog.actions.some((action) => action.id === 'services.status' && action.enabledByDefault)) {
      try { setServiceStatus((await client.getServiceStatus(activeSession)).result); } catch { setServiceStatus([]); }
    ...
    if (catalog.actions.some((action) => action.id === 'codex.projects.list' && action.enabledByDefault)) {
      try { setCodexProjects((await client.getCodexProjects(activeSession)).result); } catch { setCodexProjects([]); }
    } else setCodexProjects([]);
  ...
  }, [modeStatus?.mode, selectedThread]);
  ...
  const forget = async () => { await clearSession(); setSession(undefined); setStatus(undefined); setActions([]); setModeStatus(undefined); setModePreview(undefined); setServiceStatus([]); setCodexStatus(undefined); setCodexThreads([]); setCodexProjects([]); setCodexApprovals([]); setCodexEvents([]); setCodexResponse(''); setSelectedThread(undefined); setMessage('Session removed from secure storage.'); };
  ...
  const enabledActions = useMemo(() => actions.filter((action) => !['system.status', 'system.session.rotate', 'mode.status', 'mode.preview', 'mode.switch', 'integrations.status', 'services.status', 'codex.status', 'codex.projects.list', 'codex.threads.list', 'codex.events.list', 'codex.thread.start', 'codex.turn.start', 'codex.approvals.list', 'codex.approval.respond'].includes(action.id) && action.enabledByDefault), [actions]);
  ...
  return <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    {!session ? <View style={styles.card}><Text style={styles.heading}>Pair Android</Text><Text style={styles.body}>Device ID: {DEVICE_ID}</Text><TextInput value={pairingToken} onChangeText={setPairingToken} placeholder="Token largo o código de 6 dígitos" placeholderTextColor="#7890aa" secureTextEntry={!showPairingToken} autoCapitalize="none" autoCorrect={false} style={styles.input} /><Pressable onPress={() => setShowPairingToken((shown) => !shown)} style={styles.secondary}><Text style={styles.secondaryText}>{showPairingToken ? 'Ocultar token' : 'Mostrar token'}</Text></Pressable><Pressable disabled={busy || pairingToken.trim().length < 6} onPress={() => void pair()} style={styles.button}><Text style={styles.buttonText}>{busy ? 'Pairing…' : 'Pair device'}</Text></Pressable></View> : <View style={styles.card}>
      ...
      {serviceStatus.length ? <><View style={styles.divider} /><View style={styles.sectionHeader}><Text style={styles.heading}>Servicios registrados</Text><View style={styles.badge}><Text style={styles.badgeText}>{serviceStatus.filter((service) => service.active).length}/{serviceStatus.length} ACTIVOS</Text></View></View><Text style={styles.caption}>Unidades declaradas en Fedora; la app solo puede consultar su estado.</Text><View style={styles.integrationList}>{serviceStatus.map((service) => <Text key={service.id} style={styles.body}>{service.id} · {service.active ? 'Activo' : service.available ? 'Inactivo' : 'No disponible'} · {service.allowedModes.join('/')}</Text>)}</View></> : null}
      </> : null}
      {activeTab === 'codex' ? <View style={styles.modePanel}><View style={styles.sectionHeader}><View><Text style={styles.heading}>Codex control</Text><Text style={styles.caption}>App Server · CLI {codexStatus?.cliVersion ?? 'unknown'}</Text></View><View style={[styles.badge, !codexStatus?.connected && styles.offlineBadge]}><Text style={[styles.badgeText, !codexStatus?.connected && styles.offlineText]}>{codexStatus?.connected ? 'CONECTADO' : 'OFFLINE'}</Text></View></View><View style={styles.codexStatusLine}><View style={[styles.statusDot, codexStatus?.connected ? styles.onlineDot : styles.offlineDot]} /><Text style={styles.caption}>{codexStatus?.connected ? 'Listo para recibir tareas' : 'Fedora no responde al App Server'}</Text><Pressable disabled={busy} onPress={() => void runStatus()}><Text style={styles.refreshLink}>REINTENTAR</Text></Pressable></View>{!codexStatus?.connected ? <View style={styles.offlinePanel}><Text style={styles.offlineTitle}>Codex está offline</Text><Text style={styles.caption}>El control permanece disponible, pero no se enviarán tareas hasta que el servicio vuelva a estar conectado.</Text></View> : null}<Pressable onPress={() => setShowCodexHistory((visible) => !visible)} style={styles.historyToggle}><Text style={styles.historyToggleText}>{showCodexHistory ? '‹ OCULTAR HISTORIAL' : '› ABRIR HISTORIAL'}</Text><Text style={styles.historyToggleCount}>{codexThreads.length} THREADS</Text></Pressable>{showCodexHistory ? <View style={styles.historyPanel}><View style={styles.historyHeader}><Text style={styles.historyTitle}>THREAD HISTORY</Text><Text style={styles.caption}>Navegación</Text></View>{codexThreads.length ? codexThreads.map((thread) => <Pressable key={thread.threadId} onPress={() => { setSelectedThread(thread.threadId); setShowCodexHistory(false); }} style={[styles.historyItem, selectedThread === thread.threadId && styles.historyItemActive]}><Text numberOfLines={2} style={styles.historyItemTitle}>{thread.title ?? 'Sin título'}</Text><Text style={styles.historyItemMeta}>{thread.status.toUpperCase()}</Text><Text numberOfLines={1} style={styles.historyItemProject}>{thread.projectPath}</Text></Pressable>) : <Text style={styles.caption}>Aún no hay threads.</Text>}</View> : null}{!showCodexHistory && codexThreads.length ? codexThreads.slice(0, 3).map((thread) => <Pressable key={thread.threadId} onPress={() => setSelectedThread(thread.threadId)} style={[styles.thread, selectedThread === thread.threadId && styles.threadActive]}><View style={styles.threadHeader}><Text numberOfLines={1} style={styles.threadTitle}>{thread.title ?? 'Sin título'}</Text><Text style={styles.threadStatus}>{thread.status.toUpperCase()}</Text></View><Text style={styles.caption}>{thread.projectPath}</Text>{thread.changedFiles?.length ? <Text style={styles.caption}>Cambios: {thread.changedFiles.map((file) => `${file.kind} ${file.path}`).join(', ')}</Text> : null}</Pressable>) : null}{!codexThreads.length ? <Text style={styles.caption}>No hay threads todavía. Crea uno para comenzar.</Text> : null}
        <View style={styles.projectPicker}><View style={styles.sectionHeader}><Text style={styles.projectPickerTitle}>PROJECT CATALOG</Text><Text style={styles.historyToggleCount}>{codexProjects.length} REGISTERED</Text></View>{codexProjects.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.projectPickerRow}>{codexProjects.map((project) => <Pressable key={project.id} onPress={() => setProjectId(project.id)} style={[styles.projectChip, projectId === project.id && styles.projectChipActive]}><Text style={[styles.projectChipText, projectId === project.id && styles.projectChipTextActive]}>{project.id}</Text></Pressable>)}</ScrollView> : <Text style={styles.caption}>No hay proyectos registrados.</Text>}</View>{actions.some((action) => action.id === 'codex.thread.start' && action.enabledByDefault) ? <><TextInput value={projectId} onChangeText={setProjectId} placeholder="Project ID registrado" placeholderTextColor="#7890aa" autoCapitalize="none" style={styles.input} /><TextInput value={threadTitle} onChangeText={setThreadTitle} placeholder="Título opcional" placeholderTextColor="#7890aa" style={styles.input} /><Pressable disabled={busy} onPress={() => void startThread()} style={styles.button}><Text style={styles.buttonText}>Start Codex thread</Text></Pressable></> : null}
