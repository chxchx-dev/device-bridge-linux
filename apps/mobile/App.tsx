...
export default function App() {
  ...
  useEffect(() => {
    ...
    return connectBridgeEvents(session, (event) => {
      if (event.type === 'codex.task.event' && isCodexTaskEvent(event.payload)) { const taskEvent = event.payload; setCodexEvents((current) => current.some((item) => item.eventId === taskEvent.eventId) ? current : [...current, taskEvent].slice(-100)); }
      if (event.type === 'action.completed' || event.type === 'codex.task.updated' || event.type === 'codex.threads.updated') void refresh(session);
  ...
  const forget = async () => { await clearSession(); setSession(undefined); setStatus(undefined); setActions([]); setModeStatus(undefined); setCodexStatus(undefined); setCodexThreads([]); setCodexApprovals([]); setCodexEvents([]); setSelectedThread(undefined); setMessage('Session removed from secure storage.'); };
