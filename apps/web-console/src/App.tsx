...
type CodexThread = { threadId: string; projectPath: string; title: string | null; status: string; lastEventAt: string; lastMessage?: string | null; changedFiles?: readonly { path: string; kind: string; diff: string | null }[] };
type CodexApproval = { approvalId: string; kind: string; risk: string; summary: string; cwd: string | null };
type IntegrationStatus = { kdeConnect: { available: boolean; pairedReachable: boolean; deviceCount: number }; adb: { available: boolean; connected: boolean; deviceCount: number }; scrcpy: { available: boolean; version: string | null }; sunshine: { available: boolean; active: boolean } };
type ModeStatus = { mode: 'dev' | 'game' | null; transitioning: boolean };
...
export function App() {
  ...
  const refresh = useCallback(async (activeSession: Session) => {
    ...
    const hasAction = (id: string) => catalog.actions.some((action: Action) => action.id === id && action.enabledByDefault);
    const readAction = async (id: string) => {
      if (!hasAction(id)) return undefined;
      try { return (await request(`/v1/actions/${id}`, activeSession, { method: 'POST', body: JSON.stringify({ input: {} }) })).result; }
      catch { return undefined; }
    };
    const [integrations, mode] = await Promise.all([readAction('integrations.status'), readAction('mode.status')]);
    setIntegrationStatus(integrations); setModeStatus(mode);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onopen = () => { delay = 500; setConnectionState('connected'); setMessage('Live connection established.'); };
      socket.onmessage = (event) => { try { if (JSON.parse(event.data).type === 'action.completed') void refresh(session); } catch { /* disconnected peer */ } };
      socket.onclose = () => { if (!stopped) { setConnectionState('reconnecting'); timer = window.setTimeout(connect, delay); delay = Math.min(delay * 2, 10000); } };
      socket.onerror = () => socket.close();
    };
    connect();
    return () => { stopped = true; if (timer) window.clearTimeout(timer); socketRef.current?.close(); setConnectionState('offline'); };
  }, [refresh, session]);
 ...
 return <main className="shell"><header><div><div className="eyebrow">PRIVATE TAILNET CONTROL</div><h1>Fedora cockpit</h1></div><div className="header-actions"><span className={`pill ${connectionState === 'connected' ? 'live' : ''}`}>{connectionState === 'connected' ? 'LIVE' : connectionState === 'reconnecting' ? 'RECONNECTING' : 'OFFLINE'}</span><button className="secondary" onClick={() => setSession(undefined)}>Forget session</button></div></header><p className="message" role="status">{message}</p><section className="grid"><article className="card"><div className="card-title"><h2>System status</h2><span className="pill live">LIVE</span></div>{status ? <dl><div><dt>Host</dt><dd>{status.hostname}</dd></div><div><dt>Platform</dt><dd>{status.platform}</dd></div><div><dt>Uptime</dt><dd>{Math.floor(status.uptimeSeconds / 3600)}h {Math.floor(status.uptimeSeconds / 60) % 60}m</dd></div><div><dt>Memory</dt><dd>{Math.round((status.totalMemoryBytes - status.freeMemoryBytes) / 1024 / 1024)} / {Math.round(status.totalMemoryBytes / 1024 / 1024)} MB</dd></div></dl> : <p className="muted">Loading status…</p>}<button onClick={runStatus} disabled={busy}>Refresh status</button></article>{(integrationStatus || modeStatus) ? <article className="card"><div className="card-title"><h2>Runtime observability</h2><span className={`pill ${connectionState === 'connected' ? 'live' : ''}`}>{connectionState === 'connected' ? 'SYNCED' : 'STALE'}</span></div>{modeStatus ? <dl><div><dt>Mode</dt><dd>{modeStatus.mode?.toUpperCase() ?? 'UNKNOWN'}</dd></div><div><dt>Transition</dt><dd>{modeStatus.transitioning ? 'In progress' : 'Stable'}</dd></div></dl> : null}{integrationStatus ? <dl><div><dt>KDE Connect</dt><dd>{integrationStatus.kdeConnect.available && integrationStatus.kdeConnect.pairedReachable ? `Ready (${integrationStatus.kdeConnect.deviceCount})` : 'Unavailable'}</dd></div><div><dt>ADB</dt><dd>{integrationStatus.adb.available && integrationStatus.adb.connected ? `Connected (${integrationStatus.adb.deviceCount})` : 'Unavailable'}</dd></div><div><dt>scrcpy</dt><dd>{integrationStatus.scrcpy.available ? integrationStatus.scrcpy.version ?? 'Available' : 'Unavailable'}</dd></div><div><dt>Sunshine</dt><dd>{integrationStatus.sunshine.active ? 'Active' : integrationStatus.sunshine.available ? 'Available' : 'Unavailable'}</dd></div></dl> : null}<p className="muted">Authenticated, read-only adapter status.</p></article> : null}<article className="card"><div className="card-title"><h2>Actions</h2><span className="pill">ALLOWLISTED</span></div><p className="muted">Only declared action IDs are available. No shell commands are sent from this console.</p><button onClick={runStatus} disabled={busy}>Run system.status</button>{lockAction?.enabledByDefault && <button className="danger" onClick={lock} disabled={busy}>Lock Fedora session</button>}</article>{codexStatus ? <article className="card"><div className="card-title"><h2>Codex Cockpit</h2><span className={`pill ${codexStatus.connected ? 'live' : ''}`}>{codexStatus.connected ? 'CONNECTED' : 'OFFLINE'}</span></div><p className="muted">App Server · CLI {codexStatus.cliVersion ?? 'unknown'}</p>{codexThreads.length ? codexThreads.map((thread) => <div key={thread.threadId}><strong>{thread.title ?? thread.threadId}</strong><p className="muted">{thread.status} · {thread.projectPath}</p>{thread.changedFiles?.length ? <p className="muted">Changes: {thread.changedFiles.map((file) => `${file.kind} ${file.path}`).join(', ')}</p> : null}{thread.lastMessage ? <p>{thread.lastMessage}</p> : null}</div>) : <p className="muted">No Codex threads.</p>}{codexApprovals.length ? <div><h3>Pending approvals</h3>{codexApprovals.map((approval) => <div key={approval.approvalId} className="approval"><strong>{approval.kind} · {approval.risk}</strong><p>{approval.summary}</p><p className="muted">{approval.cwd ?? 'No working directory'}</p><span className="pill">Respond from Android</span></div>)}</div> : null}</article> : null}</section></main>;
}
